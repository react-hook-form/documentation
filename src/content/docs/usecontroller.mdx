...
## \</> `useController:` <TypeText>([UseControllerProps](/ts#UseControllerProps)) => [UseControllerReturn](/ts#UseControllerReturn)</TypeText>
...
| `exact`            | <TypeText>boolean = true</TypeText>             |          | <Since version="7.67.0" /> This prop will enable an exact match for input name subscriptions, default to true. **Note:** this differs from [`useWatch`](/docs/usewatch) and [`useFormState`](/docs/useformstate), which both default `exact` to `false`.                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `formState`  | `defaultValues`      | <TypeText>object</TypeText>               | <Since version="7.37.0" /> The value that was set in [`useForm`](/docs/useform)'s `defaultValues` or updated `defaultValues` via [`reset`](/docs/useform/reset) API.                                                                                                                                                                 |
...
| `formState`  | `isLoading`          | <TypeText>boolean</TypeText>              | <Since version="7.41.0" /> `true` if the form is currently loading async default values. **Important:** this prop is only applicable to async `defaultValues`.                                                                                                                                                                       |
...
- It's important to be aware of each prop's responsibility when working with external controlled components, such as MUI, AntD, Chakra UI. Its job is to spy on the input, report, and set its value.
  ...
  - **ref**: allow input to be focused with error — this only works if the target component forwards refs (`React.forwardRef`) or exposes an equivalent, like MUI's `inputRef`. If it doesn't accept a ref at all, omit `ref` from what you spread and handle focus manually.
...
- Do not `register` the input again. This custom hook is designed to take care of the registration process.
...
- It's best to use a single `useController` call per component — each call creates its own subscription, so multiple calls in one component can cause extra re-renders. If you need more than one controlled field in the same component, rename the destructured `field` for each call to avoid naming collisions, or consider using `Controller` instead.
