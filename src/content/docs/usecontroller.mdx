...
| `formState`  | `isValid`            | <TypeText>boolean</TypeText>              | Set to `true` if the form doesn't have any errors. `setError` immediately forces `isValid` to `false`; this value is not itself derived from validation and will be overwritten the next time validation runs (e.g. on the next `onChange`, submit, or `trigger()` call).                                                            |
