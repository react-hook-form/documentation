...
[![NPM Version](https://img.shields.io/npm/v/@hookform/lenses)](https://www.npmjs.com/package/@hookform/lenses)
[![NPM Downloads](https://img.shields.io/npm/dt/@hookform/lenses)](https://www.npmjs.com/package/@hookform/lenses)
[![GitHub Repo stars](https://img.shields.io/github/stars/react-hook-form/lenses)](https://github.com/react-hook-form/lenses)

React Hook Form Lenses is a powerful TypeScript-first library that brings the elegance of functional lenses to React Hook Form. It provides type-safe manipulation of nested form state, enabling developers to precisely control and transform complex form data with ease.
...
```tsx
const lens = useLens({
  control: form.control, // React Hook Form control from useForm()
})
```

You can also pass a dependency array to clear the lens cache and re-create all lenses when dependencies change:
...
Working with complex, deeply nested forms in React can quickly become challenging. Traditional approaches often lead to common problems:

#### 1. String-Based Field Names Are Fragile
...
#### 2. Component Reusability Is Difficult
...
#### 3. Prop Drilling and Type Safety Issues
...
function ComplexForm() {
  ...
  return <UserSection control={control} register={register} />
}

function UserSection({ control, register }: Props) {
  return <ProfileSection control={control} register={register} />
}
...
**Required.** The control object from React Hook Form's `useForm` hook. This connects your lens to the form state management system.
...
The first argument is a Proxy with a dictionary of lenses. Pay attention that lens instantiation happens only on property access. The second argument is the original lens.
...
function ArrayComponent({ lens }: { lens: Lens<{ value: string }[]> }) {
  return (
    <AnotherComponent
      lens={lens.reflect((_, l) => [{ data: l.focus("value") }])}
    />
  )
}
...
Lenses are automatically cached to prevent unnecessary component re-renders when using `React.memo`. This means that focusing on the same path multiple times will return the same lens instance:
...
```tsx
// ❌ Creates a new function on every render, breaking cache
...
```tsx
// ✅ Memoized function preserves cache
