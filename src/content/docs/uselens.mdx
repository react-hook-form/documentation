...
Creating reusable form components requires accepting a `name` prop to specify which field the component controls. However, making this type-safe in TypeScript is extremely challenging:
...
Using `useFormContext()` in reusable components tightly couples them to specific form schemas, making them less portable and harder to reuse:
...
function AddressForm() {
  ...
  return (
    <div>
      <input {...control.register('address.street')} /> {/* Fixed field paths */}
