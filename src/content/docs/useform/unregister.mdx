...
## \</> `unregister:` <TypeText>[UseFormUnregister](/ts#UseFormUnregister)</TypeText>

This method allows you to `unregister` a single input or an array of inputs. It also provides a second optional argument <Since version="7.0.0" /> to keep state after unregistering an input.
...
| <TypeText>string[]</TypeText> | `unregister(["yourDetails.lastName"])` | `{ firstName: '' }` |
...
| `keepDefaultValue` | <TypeText>boolean</TypeText> | input's `defaultValue` which defined in `useForm` will be remained. **Note:** only has an effect when the form's top-level `shouldUnregister` option is `false` (the default); it is a no-op when `shouldUnregister: true`.   |
| `keepIsValidating` | <TypeText>boolean</TypeText> | <Since version="7.51.0" /> `validatingFields`/`isValidating` state for the unregistered field will be retained rather than cleared.                                                                                           |
