...
## \</> `resetField:` <TypeText>[UseFormResetField](/ts#UseFormResetField)</TypeText>
...
After invoking this function, with default options:

- `isValid` will be reevaluated, unless `keepError: true` is passed.
- `isDirty` will be reevaluated, unless `keepDirty: true` is passed.
