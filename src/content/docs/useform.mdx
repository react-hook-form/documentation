...
| [progressive](#progressive)                             | Forward validation attributes (`required`, `min`, `max`, etc.) as native HTML attributes on inputs. Required when using `shouldUseNativeValidation` so the browser can display built-in validation UI. Also enables progressive enhancement for the `Form` component. |
...
The `validate` function receives `{ formValues, formState, eventType, name }` and must return one of: `true` (valid), a `string` (surfaced at `formState.errors.form`), or an object of `{ [fieldName]: { type, message } }` (each entry surfaced at `formState.errors.form.<fieldName>`). It can run alongside a `resolver`.
...
- Setting `shouldUnregister: true` makes your form behave more closely to native forms.
  ...
  - Unmounted inputs must be notified at either `useForm` or `useWatch`'s `useEffect` for the hook form to verify that the input is unmounted from the DOM.
    ...
    const App = () => {
      const [showNotWork, setShowNotWork] = React.useState(false)
...
- **`progressive: true` is required alongside this option.** Without it, React Hook Form does not forward validation attributes (e.g. `required`, `minLength`) as native HTML attributes on the input, so the browser has nothing to validate against and no native UI is shown.

  ```javascript
  useForm({
    shouldUseNativeValidation: true,
    progressive: true, // required — forwards validation rules as native HTML attributes
  })
  ```
...
Forwards validation rules (`required`, `min`, `max`, `minLength`, `maxLength`, `pattern`) from [`register`](/docs/useform/register) as native HTML attributes on the input. Required alongside [`shouldUseNativeValidation`](#shouldUseNativeValidation) so the browser has something to validate against. Also enables progressive enhancement for the [`Form`](/docs/useform/form) component (rendering usable HTML before hydration).
