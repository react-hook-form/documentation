...
<SelectNav
  options={[
    ...
    {
      label: "resetDefaultValues",
      value: "/docs/useform/resetdefaultvalues",
    },
    {
      label: "setError",
...
## \</> `useForm:` <TypeText>[`UseFormProps`](/ts#UseFormProps)</TypeText>
...
| Option                                                  | Description                                                                                                                                                                                           |
| ------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [mode](#mode)                                           | Validation strategy for **before** submission behavior.                                                                                                                                               |
| [reValidateMode](#reValidateMode)                       | Validation strategy for **after** submission behavior.                                                                                                                                                |
...
| [resetOptions](#resetOptions)                           | Option to reset form state when updating with new form values.                                                                                                                                        |
...
| [progressive](#progressive)                             | Forward validation attributes (`required`, `min`, `max`, etc.) as native HTML attributes on inputs. Enables progressive enhancement for the `Form` component (usable HTML before hydration/JS loads). |
| [disabled](#disabled)                                   | Disable the entire form with all associated inputs.                                                                                                                                                   |
| [formControl](/docs/createFormControl)                  | Supply a pre-created form control object (from `createFormControl`) instead of letting `useForm` create one internally.                                                                               |
...
This option allows you to configure the validation strategy for before a user submits the form. Validation occurs during the `onSubmit` event, which is triggered by invoking the [`handleSubmit`](/docs/useform/handlesubmit) function.
...
This option allows you to configure the validation strategy for when inputs with errors get re-validated **after** a user submits the form (the `onSubmit` event occurs and the [`handleSubmit`](/docs/useform/handlesubmit) function is executed). By default, re-validation occurs during the input change event.

**Note:** <Since version="7.56.0" /> Both `mode` and `reValidateMode` are reactive — you can update them after form initialization and the new strategy will take effect on subsequent validations.
...
#### values: <TypeText>FieldValues</TypeText> <Since version="7.41.0" /> {#values}
...
#### errors: <TypeText>FieldErrors</TypeText> <Since version="7.49.0" /> {#errors}

---

The `errors` prop will react to changes and update the server errors state, which is useful when your form needs to be updated with errors returned from the server.
...
#### resetOptions: <TypeText>[KeepStateOptions](/ts#KeepStateOptions)</TypeText> <Since version="7.41.0" /> {#resetOptions}
...
- Only works when the field's `ref` is actually attached to a DOM element — e.g. `<input {...register('test')} />`. A custom-registered input whose returned props are never spread onto a real element (so no `ref` is attached) won't receive focus, since there's nothing to focus.
- The focus order is based on the `register` order.

</Admonition>

#### delayError: <TypeText>number</TypeText> <Since version="7.12.0" /> {#delayError}

---

|                                                                                                                                                                                                                        |                                                                        |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| This configuration delays the display of error states to the end user by a specified number of milliseconds. If the user corrects the input with errors, the error is removed instantly, and the delay is not applied. | <CodeSandbox url="https://codesandbox.io/s/useform-delayerror-q6c2d"/> |

#### validate: <TypeText>Function</TypeText> <Since version="7.72.0" /> {#validate}
...
The `validate` function receives `{ formValues, formState, eventType, name }` and must return one of: `true` (valid), a `string` (surfaced at `formState.errors.form`), or an object of `{ [fieldName]: { type, message } }` (each entry surfaced at `formState.errors.form.<fieldName>`). **Note:** `validate` does not run when a `resolver` is configured — `resolver` and `validate` are mutually exclusive, and only `resolver` runs if both are set.
...
By default, an input value will be retained when an input is removed. However, you can set `shouldUnregister` to `true` to `unregister` the input during unmount.
...
#### shouldUseNativeValidation: <TypeText>boolean = false</TypeText> <Since version="7.9.0" /> {#shouldUseNativeValidation}

---

This config will enable [browser native validation](https://developer.mozilla.org/en-US/docs/Learn/Forms/Form_validation). It will also enable CSS selectors `:valid` and `:invalid`, making input styling easier. You can still use these selectors even when client-side validation is disabled.

- Only works with `onSubmit` and `onChange` modes, as the `reportValidity` execution will focus the error input.
- Each registered field's validation message must be a string to be displayed natively.
- This feature only works with the `register` API and&nbsp; `useController/Controller` that are connected with actual DOM references.
- `shouldUseNativeValidation` works independently of `progressive` — it drives the browser's Constraint Validation API (`setCustomValidity`/`reportValidity`) directly from React Hook Form's own computed validation results, regardless of whether validation attributes are forwarded as native HTML attributes. Pair it with [`progressive`](#progressive) if you also want the constraint attributes themselves (`required`, `minLength`, etc.) present on the DOM, e.g. for SSR or pre-hydration fallback validation.
...
#### progressive: <TypeText>boolean = false</TypeText> <Since version="7.44.0" /> {#progressive}

---

Forwards validation rules (`required`, `min`, `max`, `minLength`, `maxLength`, `pattern`) from [`register`](/docs/useform/register) as native HTML attributes on the input. Enables progressive enhancement for the [`Form`](/docs/useform/form) component (rendering usable HTML before hydration). Independent of [`shouldUseNativeValidation`](#shouldUseNativeValidation), which drives the browser's Constraint Validation API from React Hook Form's own validation results regardless of this option — pair the two if you also want the constraint attributes present on the DOM.
...
#### disabled: <TypeText>boolean = false</TypeText> <Since version="7.48.0" /> {#disabled}
...
| `options` | <TypeText><pre>{JSON.stringify({ criteriaMode: "string", fields: "object", names: "string[]" }, null,2)}</pre></TypeText> | This is the options object containing information about the validated fields, names and `criteriaMode` from `useForm`.                      |
...
- Re-validation of an input will only occur one field at a time during a user’s interaction. The library itself will evaluate the `error` object to trigger re-renders accordingly.
...
In a future major release, the `useForm` return value will be memoized to optimize performance and reflect changes in the `formState`. Once that ships, the returned object will get a new reference whenever `formState` updates — so depending on it directly in a `useEffect` dependency array will re-run that effect on every `formState` change, which can create an infinite loop if the effect itself updates the form (e.g. calling `reset`).

<Admonition type="warning" >

Avoid depending on the entire `useForm` return value going forward — the following pattern will break once memoization ships:
...
Passing only the relevant methods, as shown below, should avoid this kind of issue:
...
The recommended way is to pass destructured methods to the dependencies of a `useEffect`
