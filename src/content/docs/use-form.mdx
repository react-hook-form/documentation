...
> :memo: **Note:** only registered fields with a `ref` will work. Custom registered inputs do not apply. For example: `register('test') // doesn't work`

> :memo: **Note:** the focus order is based on the `register` order.
...
> :bulb: **Tip:** You can debug your schema via the following code snippet:
