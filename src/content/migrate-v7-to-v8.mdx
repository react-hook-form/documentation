...
// After (V8) — primitive values are supported directly
...
// Before (V7) — ref was a partial ref-like object
ref.current // not the actual DOM node

// After (V8) — ref is the actual input element
ref.current // the real <input> DOM node
```

If you were reading properties off `register(...).ref` directly (rather than spreading `{...register("firstName")}` onto an `<input>`), update your code to expect the real DOM element.
