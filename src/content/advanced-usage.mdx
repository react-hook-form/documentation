...
When we are building forms, there are times when our input lives inside deeply nested component trees; that's when [FormContext](/docs/useformcontext) comes in handy. However, we can further improve the developer experience by creating a `ConnectForm` component and leveraging React's [renderProps](https://reactjs.org/docs/render-props.html). The benefit is that you can connect your inputs to React Hook Form much more easily.
...
// we can use React.memo to prevent re-renders except when the isDirty state changes
